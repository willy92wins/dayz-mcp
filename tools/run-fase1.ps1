...
function Info($Message) {
  Write-Host "[fase1] $Message" -ForegroundColor Cyan
}
...
$VerdictFile = Join-Path $PSScriptRoot "fase1-verdict.json"
$RunId = Get-Date -Format "yyyyMMdd_HHmmss"
$WorkRoot = Join-Path (Join-Path $ProjectDev "_fase1") "run_$RunId"
...
$KeyFile = Join-Path $WorkRoot "fase1.key"
...
try {
  ...
  if ($playerReady) {
    $clientPyArgs = "`"$ClientPy`" --mode phase1 --port $Port --keyfile `"$KeyFile`" --output `"$VerdictFile`" --timeout $ClientTimeoutSeconds"
    Info "running phase1 client"
    ...
    Info "player did not become ready within ${WaitInGameSeconds}s; skipping phase1 client suite"
  }
} finally {
  if (-not $NoStop) {
...
try {
  if ($verdictText) {
    $phase1Verdict = $verdictText | ConvertFrom-Json
    if ($phase1Verdict.b3_decision_data) {
      $b3DecisionDataText = $phase1Verdict.b3_decision_data | ConvertTo-Json -Depth 20
    }
  }
} catch {
  $b3DecisionDataText = "Could not parse b3_decision_data: $($_.Exception.Message)"
}
...
Write-Host "===== FASE1 CLIENT STDOUT BEGIN ====="
if ($clientStdoutText) { Write-Host $clientStdoutText }
Write-Host "===== FASE1 CLIENT STDOUT END ====="
if ($clientStderrText) {
  Write-Host "===== FASE1 CLIENT STDERR BEGIN ====="
  Write-Host $clientStderrText
  Write-Host "===== FASE1 CLIENT STDERR END ====="
}
Write-Host "===== MCP MARKERS BEGIN ====="
if ($markers) { Write-Host $markers }
Write-Host "===== MCP MARKERS END ====="
Write-Host "===== B3 DECISION DATA BEGIN ====="
if ($b3DecisionDataText) { Write-Host $b3DecisionDataText }
Write-Host "===== B3 DECISION DATA END ====="
Write-Host "===== LOG EVIDENCE BEGIN ====="
if ($evidence) { Write-Host $evidence }
Write-Host "===== LOG EVIDENCE END ====="
Write-Host "===== FASE1 VERDICT BEGIN ====="
if ($verdictText) { Write-Host $verdictText }
Write-Host "===== FASE1 VERDICT END ====="
