...
function Info($Message) {
  Write-Host "[fase2] $Message" -ForegroundColor Cyan
}
...
$VerdictFile = Join-Path $PSScriptRoot "fase2-verdict.json"
$RunId = Get-Date -Format "yyyyMMdd_HHmmss"
$WorkRoot = Join-Path (Join-Path $ProjectDev "_fase2") "run_$RunId"
...
$KeyFile = Join-Path $WorkRoot "fase2.key"
...
try {
  ...
  if ($playerReady) {
    $clientPyArgs = "`"$ClientPy`" --mode phase2 --port $Port --keyfile `"$KeyFile`" --spawn `"$spawnActual`" --output `"$VerdictFile`" --timeout $ClientTimeoutSeconds"
    Info "running phase2 client"
    ...
    Info "player did not become ready within ${WaitInGameSeconds}s; skipping phase2 client suite"
  }
} finally {
  if (-not $NoStop) {
...
$phase2NormalComparisonText = ""
try {
  if ($verdictText) {
    $phase2Verdict = $verdictText | ConvertFrom-Json
    if ($phase2Verdict.normal_comparison) {
      $phase2NormalComparisonText = $phase2Verdict.normal_comparison | ConvertTo-Json -Depth 20
    }
  }
} catch {
  $phase2NormalComparisonText = "Could not parse normal_comparison: $($_.Exception.Message)"
}
...
Write-Host "===== FASE2 CLIENT STDOUT BEGIN ====="
if ($clientStdoutText) { Write-Host $clientStdoutText }
Write-Host "===== FASE2 CLIENT STDOUT END ====="
if ($clientStderrText) {
  Write-Host "===== FASE2 CLIENT STDERR BEGIN ====="
  Write-Host $clientStderrText
  Write-Host "===== FASE2 CLIENT STDERR END ====="
}
Write-Host "===== MCP MARKERS BEGIN ====="
if ($markers) { Write-Host $markers }
Write-Host "===== MCP MARKERS END ====="
Write-Host "===== PHASE2 NORMAL COMPARISON BEGIN ====="
if ($phase2NormalComparisonText) { Write-Host $phase2NormalComparisonText }
Write-Host "===== PHASE2 NORMAL COMPARISON END ====="
Write-Host "===== LOG EVIDENCE BEGIN ====="
if ($evidence) { Write-Host $evidence }
Write-Host "===== LOG EVIDENCE END ====="
Write-Host "===== FASE2 VERDICT BEGIN ====="
if ($verdictText) { Write-Host $verdictText }
Write-Host "===== FASE2 VERDICT END ====="
