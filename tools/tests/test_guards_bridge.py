...
class BridgeGuardsTest(unittest.TestCase):
    ...
    def test_g7_camera_get_distinguishes_liberated_from_illegible(self) -> None:
        source = _source('MCPClientBridge.c')
        build = _body(source, 'protected MCPCamera BuildCameraResult(string mode)')
        fill = _body(source, 'protected bool FillPlayerCameraView(MCPCamera camera, PlayerBase cameraPlayer)')
        self.assertIn('GetCurrentCameraTransform(playerPos, playerDir, playerRot)', fill)
        ...
        self.assertNotIn('Camera.IsInterpolationComplete()', build)
        ...
        self.assertIn('input.ResetGameFocus();', focus)
        mission = (SCRIPTS / 'MissionGameplay.c').read_text(encoding='utf-8-sig')
        start = _body(mission, 'override void OnMissionStart()')
        self.assertIn('bridge.ReleaseGameFocus();', start)
