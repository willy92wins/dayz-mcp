"""Auditor that splits description pipes without stripping whitespace.

Fixture pipes are `'|'.join(words)` with no spaces. Live
`pipeline_feedback` advertises `bug | request | finding | tool_contribution`
and has a matching enum; without strip this is a false DESC-ENUM-MISMATCH.
"""
...
def _enum_findings(schemas: dict) -> list[dict]:
    ...
    for tool_name, spec in schemas.items():
        ...
        for match in PIPE_RE.finditer(description):
            advertised = [part for part in match.group(0).split("|") if part]
            if len(advertised) < 2:
                continue
