"""Resolver that only reads anyOf/oneOf alternatives shaped like the fixtures.

Fixture union branches are exactly `{type}` or `{type, enum}`. Live marker
and optional-array params add `additionalProperties` or `items`; those
branches are skipped, so `logs_since.marker` loses `object`.
"""
...
_FIXTURE_ALT_KEYS = frozenset({"type", "enum"})
    ...
    for union_key in ("oneOf", "anyOf"):
        ...
        for alt in alts:
            if not isinstance(alt, dict):
                continue
            if not set(alt.keys()) <= _FIXTURE_ALT_KEYS:
                continue
            item = alt.get("type")
            if isinstance(item, str) and item != "null" and item not in collected:
                collected.append(item)
