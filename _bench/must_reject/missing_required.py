"""Resolver that requires the JSON Schema `required` key to exist.

Fixtures always emit a non-empty `required` list. Seventeen live tools omit
the key entirely (every param optional). `schema["required"]` then KeyErrors.
"""
...
async def _index_tools() -> dict[str, dict]:
    ...
    for tool in listed:
        ...
        required = {
            item for item in schema["required"] if isinstance(item, str)
        }
