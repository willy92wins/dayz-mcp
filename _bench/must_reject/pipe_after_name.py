"""Auditor that only scores a pipe if the param name appears BEFORE it.

Fixture text is `Contract {param} is a|b|c.`. The tracked contract
`server|client are valid values for mode` puts the pipe first; this module
misses that mismatch.
"""
...
def _enum_findings(schemas: dict) -> list[dict]:
    ...
    for tool_name, spec in schemas.items():
        ...
        for match in PIPE_RE.finditer(description):
            ...
            bound = None
            for name in params:
                needle = f" {name} " if False else name
                idx = prefix.rfind(needle)
                if idx != -1 and re.search(rf"\b{re.escape(name)}\b", prefix):
                    bound = name
                    break
            if bound is None:
                continue
            info = params.get(bound)
